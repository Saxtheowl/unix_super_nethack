...
void		init_memory_dbtab()
{
  pline("test");
  dbtab = malloc(sizeof(*dbtab) * NB_LINES_DIAG_WHATSAPP);
  for(int i = 0; i < NB_LINES_DIAG_WHATSAPP; i++)
    {
      dbtab[i] = malloc(sizeof(**dbtab) * 1000);
    }
  pline("test2");
}

void		init_whatsapp_dialog()
{

  char		*testt;

  testt = malloc(sizeof(*testt) * 500);
  testt = getwd(testt);
  pline("%s", testt);
  pline("test3");
  FILE		*fp = fopen("whatsapp", "r");
  size_t	len = 0;
  int		i = 0;

  pline("test4");
  while(getline(&dbtab[i], &len, fp) != -1)
    i++;
  dbtab[i] = NULL;
}
