...
#define NB_LINES_DIAG_WHATSAPP 7600

boolean		is_Manu(int nb)
{
  if(nb == 361 || nb == 362)
    return(1);
  else
...
domove()
{
    ...
    if (is_safepet(mtmp) && !(is_hider(mtmp->data) && mtmp->mundetected)) {
        ...
        if (mtmp->mtrapped && (trap = t_at(mtmp->mx, mtmp->my)) != 0
            ...
            newsym(u.ux0, u.uy0);
	    int	dat_rdm = rn2(200);
	    if(is_Manu(mtmp->mnum) == 1 && dat_rdm > 0 && dat_rdm < 10)
	      {
		if(dat_rdm == 1)
		  pline("%s spank you gently", y_monnam(mtmp));
		else if(dat_rdm == 2)
		  pline("%s spank you!", y_monnam(mtmp));
		else if(dat_rdm == 3)
		  pline("%s spank you loudly!", y_monnam(mtmp));
		else if(dat_rdm == 4)
		  pline("%s stroke your ass as you pass by", y_monnam(mtmp));
		else if(dat_rdm == 5)
		  pline("%s put a kiss on your neck", y_monnam(mtmp));
		else if(dat_rdm == 6)
		  pline("%s let you move then stare at your ass for a moment", y_monnam(mtmp));
		else if(dat_rdm == 7)
		  pline("%s was looking at you with envy in the eyes then realize that you want to move and let you pass", y_monnam(mtmp));
		else if(dat_rdm == 8)
		pline("%s look at you like he was wondering whats behind your dress", y_monnam(mtmp));
		else if(dat_rdm == 9)
		pline("%s kiss your forehead", y_monnam(mtmp));
	      }
	    else
